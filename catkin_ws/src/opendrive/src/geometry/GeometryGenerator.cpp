  ...
  namespace geometry
  {
    ...
    bool generateRoadGeometry(RoadInformation &roadInfo, opendrive::OpenDriveData &mapData)
    {
      ...
      for (auto it = laneSections.begin(); it != laneSections.end(); it++)
      {
        ...
        auto inRange = [&s0, &s1](double const &s)
        { return (s < s1) && (s > s0); };
    ...
    bool convertToGeoPoints(opendrive::OpenDriveData &mapData)
    {
      ...
      if (projPtr == nullptr)
      {
        ...
        std::string defaultProjection = "+proj=tmerc +ellps=WGS84 +lon_0=" + std::to_string(mapData.geoReference.longitude) + " +lat_0=" + std::to_string(mapData.geoReference.latitude);
